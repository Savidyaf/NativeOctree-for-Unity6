...
{
    /// <summary>
    /// Burst-compatible lookup tables for morton code encoding and octree depth calculations.
    /// Uses <see cref="SharedStatic{T}"/> so Burst-compiled code can read these without
    /// crossing the managed/native boundary.
    /// </summary>
    internal static class LookupTables
    {
        struct MortonLookupKey {}
        struct DepthSizeLookupKey {}
        struct DepthLookupKey {}

        internal unsafe struct MortonLookupData
        {
            public fixed uint Values[256];
        }

        internal unsafe struct DepthSizeLookupData
        {
            public fixed int Values[10];
        }

        internal unsafe struct DepthLookupData
        {
            public fixed int Values[9];
        }

        internal static readonly SharedStatic<MortonLookupData> MortonLookup =
            SharedStatic<MortonLookupData>.GetOrCreate<MortonLookupKey>();

        internal static readonly SharedStatic<DepthSizeLookupData> DepthSizeLookup =
            SharedStatic<DepthSizeLookupData>.GetOrCreate<DepthSizeLookupKey>();
        ...
        internal static unsafe void Initialize()
        {
            ...
            fixed (uint* dst = morton.Values)
            {
                for (int i = 0; i < 256; i++)
                    dst[i] = mortonSrc[i];
            }

            ref var depthSize = ref DepthSizeLookup.Data;
            fixed (int* dst = depthSize.Values)
            {
                dst[0] = 0;
                dst[1] = 1;
                dst[2] = 1 + 2 * 2 * 2;
                dst[3] = 1 + 2 * 2 * 2 + 4 * 4 * 4;
                dst[4] = 1 + 2 * 2 * 2 + 4 * 4 * 4 + 8 * 8 * 8;
                dst[5] = 1 + 2 * 2 * 2 + 4 * 4 * 4 + 8 * 8 * 8 + 16 * 16 * 16;
                dst[6] = 1 + 2 * 2 * 2 + 4 * 4 * 4 + 8 * 8 * 8 + 16 * 16 * 16 + 32 * 32 * 32;
                dst[7] = 1 + 2 * 2 * 2 + 4 * 4 * 4 + 8 * 8 * 8 + 16 * 16 * 16 + 32 * 32 * 32 + 64 * 64 * 64;
                dst[8] = 1 + 2 * 2 * 2 + 4 * 4 * 4 + 8 * 8 * 8 + 16 * 16 * 16 + 32 * 32 * 32 + 64 * 64 * 64 + 128 * 128 * 128;
                dst[9] = 1 + 2 * 2 * 2 + 4 * 4 * 4 + 8 * 8 * 8 + 16 * 16 * 16 + 32 * 32 * 32 + 64 * 64 * 64 + 128 * 128 * 128 + 256 * 256 * 256;
            }

            ref var depth = ref DepthLookup.Data;
            fixed (int* dst = depth.Values)
            {
                dst[0] = 0;
                dst[1] = 2;
                dst[2] = 4;
